...
roiproc = pe.Workflow(name='adhd200roisyeo')
...
datasink.inputs.base_directory = os.path.join(basedir, 'rois17')
...
datasink2.inputs.base_directory = os.path.join(basedir, 'homology_workdir', 'inputyeo')
roiproc.connect(roistripper, 'roi_file', datasink2, '@strippedroi')

def get_substitutions(subject_id):
    subs = [('_%s.dat'%subject_id, '.dat'),
        ('aparc+yeo17_warped_avgwf','%s_roi'%subject_id),
        ('aparc+yeo17_warped','%s_labels'%subject_id)
        ]
    return subs
roiproc.connect(inputnode, ('subject_id', get_substitutions), datasink, 'substitutions')

roiproc.base_dir = os.path.join('/mindhive/scratch/satra/ADHD200/working')
